...
<asp:Content ID="indexContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
            ...
            $("#list").jqGrid({
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        var cl = ids[i];
                        var be = "<input type='button' value='T' tooltips='Tambah Komisi Tindakan' onClick=\"OpenPopup('" + cl + "');\" />";
                        //                        alert(be);
                        $(this).setRowData(ids[i], { act: be });
                    }
                },
                subGrid: true,
                subGridUrl: '<%= Url.Action("GetListForSubGrid", "ActionComm") %>',
                subGridModel: [{ name: [ 'Kode Tindakan', 'Tindakan', 'Jenis Komisi', 'Komisi', 'Deskripsi'],
                    width: [ 55, 55, 80, 80, 80],
        ...
        {
            $("#popup_frame").attr("src", "<%= Url.Action("PopupAdd", "ActionComm") %>/"+id);
            $("#popup").dialog("open");
            return false;   
        }
    </script>
